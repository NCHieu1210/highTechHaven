...
import { deleteRangeBlogService } from "../../services/blogsService";


const DeleteRangeBlgos = (props) => {
  ...
  const handleDelete = async () => {
    setLoading(true);
    try {
      const response = await deleteRangeBlogService(selectedRowKeys);
      if (response.success) {
  ...
  return (
    <>
      ...
      <Popconfirm
        title={`Xóa ${selectedRowKeys.length} chuyên mục!`}
...
export default DeleteRangeBlgos;