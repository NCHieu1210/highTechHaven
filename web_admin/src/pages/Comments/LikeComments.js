...
const LikeComments = (props) => {
  const { id, likedComment, quantityLiked } = props;
  ...
  return (
    <Space.Compact className="comments__func">
      <span style={{ color: 'red', marginRight: '5px' }}>{quantityLiked && quantityLiked}</span>
      <Button
        className="comments__func--like"
        shape="circle"
        title="Thích"
        onClick={() => handleLikeComment(id)}
      >
        {likedComment && likedComment.find(lkc => lkc.commentID === id) ?
