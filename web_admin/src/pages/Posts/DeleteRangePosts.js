...
import { deleteRangePostsService } from "../../services/postsService";

const DeleteRangePosts = (props) => {
  ...
  const handleDelete = async () => {
    setLoading(true);
    try {
      const response = await deleteRangePostsService(selectedRowKeys);
      if (response.success) {
        Swal.fire({
          title: "Thành công!",
  ...
  return (
    <>
      ...
      <Popconfirm
        title={`Xóa ${selectedRowKeys.length} bài viết!`}
...
export default DeleteRangePosts;